# General
CMD      :=
PKG      := github.com/thisissoon/datastore-exporter
PKG_LIST := $(shell go list ${PKG}/... | grep -v /vendor/)

# Docker
DOCKER_REGISTRY = datastore-exporter
...
BIN_NAME   ?= datastore-exporter-$(shell go env GOOS)-$(shell go env GOARCH)
ifeq ("$(GIT_TAG)","")
	BIN_VERSION = $(GIT_SHA)
endif
BIN_VERSION ?= ${GIT_TAG}
...
# Build a binary
.PHONY: build
build: CMD = ./cmd/datastore-exporter
